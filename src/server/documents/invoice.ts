...
/** Комплектовочный лист для склада: что собрать и куда отгрузить. Цены не нужны. */
export function buildPackingList(order: OrderDetails): TDocumentDefinitions {
  const right: Alignment = "right";
  ...
  return {
    pageSize: "A4",
    pageMargins: [40, 40, 40, 40],
    info: { title: `Комплектовочный лист № ${order.number}` },
    content: [
      {
        text: `Комплектовочный лист к заказу № ${order.number} от ${formatMoscowDate(order.createdAt)}`,
        style: "title",
      },
      {
        text: [
      ...
      },
      {
        text: [
          ...
          order.deliveryMethod ? deliveryLabels[order.deliveryMethod] : "не выбрана",
        ...
        table: {
          ...
          body: [
            [
              ...
              { text: "Наименование", style: "th" },
              { text: "Кол-во", style: "th", alignment: right },
              { text: "Собрано", style: "th", alignment: center },
            ],
            ...order.items.map((item, index): TableCell[] => [
              ...
              { text: item.name },
              { text: String(item.quantity), alignment: right },
              // Пустая клетка для отметки рукой при сборке.
      ...
      {
        text: order.customerComment ? `Комментарий клиента: ${order.customerComment}` : "",
        margin: [0, 12, 0, 0],
        style: "small",
      },
      {
        text: "Собрал ______________________     Проверил ______________________",
        margin: [0, 30, 0, 0],
        style: "small",
      },
    ],
