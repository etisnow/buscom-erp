import "server-only";
import { z } from "zod";
import { assignSkus } from "@/domain/product/site-catalog";
...
export type SiteProductRow = z.infer<typeof siteProductRowSchema>;
 ...
 * повторный прогон обновляет название, артикул, категорию и цену по сайту.
...
export async function importSiteProducts(
  ...
  const existing = await db.product.findMany({
    select: { id: true, sku: true, externalId: true, name: true, category: true, priceKopecks: true, isActive: true },
  });
  ...
  for (const row of rows) {
    ...
    if (!current) {
      report.создано += 1;
      if (!options.dryRun)
        await db.product.create({ data: { ...data, externalId: row.externalId, compatibility: [] } });
      continue;
    }
    ...
    report.обновлено += 1;
    if (!options.dryRun) await db.product.update({ where: { id: current.id }, data });
  }
