...
1. Каталог, синхронизируемый с сайта: артикул, название, категория (справочник-дерево «раздел → подкатегория», экран `/products/categories`), цена, совместимость — модели авто. Список моделей взят с сайта поставщика vanproject.ru (2026-09-24):
...
1. Внутренние: Telegram-бот для команды — «новый заказ».
...
2. Справочники: способы доставки и оплаты, причины отмены, SLA по статусам (скрыт, пока SLA выключен), **источники заказов** (редактируемый список; «Сайт» и «Прежняя ERP» — системные, их только переименовывают). Источник заказа, заведённого руками, меняется в карточке с записью в журнал.
...
| 4. Расширение          | обратная синхронизация с сайтом, API ТК, УПД и товарный чек, возвраты | ERP — единый источник правды                         |
