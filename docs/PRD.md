...
- [ ] В той же форме заводятся адреса доставки — сколько нужно, один помечается «по умолчанию» (без пометки им становится первый). Клиента не с сайта заводят целиком и сразу, вторым заходом в карточку ходить не нужно.
- [ ] Телефон нормализуется к `+7XXXXXXXXXX` — так же, как при приёме заказа.
- [ ] Дубль не создаётся: если телефон или email совпал с существующим клиентом, форма показывает, кто это, и ведёт в его карточку. Правило совпадения то же, что в «Бизнес-правилах» — сначала телефон, потом email.
