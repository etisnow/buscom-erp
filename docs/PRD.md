...
5. В заказе с товарами поставщиков у **каждого поставщика свой трек** по его цепочке — это подстатусы заказа, пока он в работе («В работе», «Ждёт оплаты», «Оплачен»). Этапы проходят по порядку, шаг назад разрешён для исправления. Каждая смена этапа пишется в журнал заказа.
...
| Возврат в работу           | AWAITING_PAYMENT → IN_PROGRESS разрешён (клиент меняет состав); отмена из PAID и SHIPPING — только HEAD/ADMIN                                                                          | —             |
...
stateDiagram-v2
    ...
    IN_PROGRESS --> SHIPPING: постоплата
    PAID --> SHIPPING
    SHIPPING --> SHIPPED: трек / самовывоз
    SHIPPED --> COMPLETED
    NEW --> CANCELLED
    IN_PROGRESS --> CANCELLED
    AWAITING_PAYMENT --> CANCELLED
    PAID --> CANCELLED: только HEAD/ADMIN
    SHIPPING --> CANCELLED: только HEAD/ADMIN
    COMPLETED --> [*]
    CANCELLED --> [*]
```
...
| PAID             | Оплачен              | менеджер                  | 1 раб. день      |
| SHIPPING         | Отправка             | менеджер                  | 2 раб. дня       |
| SHIPPED          | Отгружен             | менеджер                  | —                |
| COMPLETED        | Выполнен             | —                         | —                |
| CANCELLED        | Отменён (с причиной) | —                         | —                |

**Подстатусы поставщиков (M9).** Пока заказ в «В работе», «Ждёт оплаты» или «Оплачен», у каждого его поставщика идёт свой трек по цепочке этапов из карточки поставщика. Переход в «Отправку» (и из «Оплачен», и постоплатой из «В работе») сервер разрешает, только когда каждый трек стоит на последнем этапе; поставщик без цепочки заказ не держит. После «Отправки» и в финальных статусах этапы не меняются.
