...
2. Поля: способ, адрес/терминал, стоимость, габариты и вес груза, трек-номер, дата отгрузки. **Стоимость доставки в сумму заказа не входит** (решение владельца, 2026-09-25): клиент оплачивает доставку транспортной компании сам. Она видна в блоке «Доставка» и справкой под итогами, но не попадает ни в итог заказа, ни в счёт, ни в статус оплаты. «Итого» заказа с сайта сверяется с суммой ERP плюс доставка — в OpenCart доставка входит в итог. Интеграция с API ТК — после MVP. Вес вводится в килограммах до граммов, габариты — длина × ширина × высота в целых сантиметрах; всё необязательно. Дата отгрузки правится в карточке, а если пуста — ставится сама при переходе в «Выполнен» (сделано 2026-09-24).
...
2. **Переписка с клиентом по почте.** С 2026-09-25 (решение владельца) **письма не привязываются к заказам — переписка принадлежит клиенту.** В карточке заказа — последние 10 писем переписки с клиентом заказа, более старые подгружаются по 10 при прокрутке вверх (и форма письма с шаблонами); в карточке клиента — вся переписка. Переписка клиента — письма, где он указан, и письма с его адреса или на его адрес. Клиент входящего определяется по цепочке, номеру заказа на сайте в теме (клиент этого заказа) или адресу; не определился — письмо во вкладке «Почта → Без клиента», где его привязывают к клиенту поиском. В живом ящике — «Добавить в переписку». Пункты ниже, где говорится о привязке к заказу, заменены этим.
   Исходное описание (2026-09-24): Общий ящик — `info@bus-com.ru`: в него же приходят заказы с сайта. Письмо о заказе с сайта заводит заказ, любое другое ложится в переписку.
...
**M10. Чат сотрудников (P2, добавлен 2026-09-25)**

1. Раздел «Чат» (`/chat`, все роли) — один общий канал на всех сотрудников. Личных сообщений и обсуждений внутри заказа нет (решение владельца).
2. Лента обновляется сама раз в несколько секунд, пока вкладка открыта; мгновенная доставка не нужна. Старые сообщения — «Показать раньше» по 50.
3. У пункта меню — число непрочитанных (чужие сообщения после последнего просмотра ленты); обновляется само, гаснет, когда ленту посмотрели.
4. Своё сообщение можно исправить (помечается «изменено») и удалить; администратор удаляет и чужое. Удалённое остаётся в ленте пометкой «Сообщение удалено», текст и файлы стираются.
5. Вложения: до 10 файлов и 15 МБ на сообщение, любые типы; кнопка-скрепка, перетаскивание, вставка из буфера. Картинки видны в ленте, PDF и картинки открываются во вкладке, прочее скачивается. **Файлы хранятся год**, потом стираются, в ленте остаётся имя с пометкой.
6. Ссылка на заказ: «№3021», «№ 3021» или «#3021» в тексте становится ссылкой на карточку, если такой заказ есть.
7. **Пуш-уведомления на телефон** (2026-09-25): о новом сообщении — всем, кроме автора, даже когда ERP закрыта. Включаются в «Настройках» на каждом устройстве отдельно; там же «Проверить» и «Выключить». Android — в Chrome без установки; iPhone — только iOS 16.4+ и только ERP, добавленная на главный экран («Поделиться → На экран Домой»), инструкция показывается в настройках. Уведомления о чате заменяют друг друга, а не копятся; нажатие открывает чат.
8. **Ответить** (2026-09-25): в меню «…» любого сообщения. Над полем ввода — плашка с цитатой и крестиком «Не отвечать»; в ленте ответ показывает автора и начало исходного сообщения, нажатие прокручивает к нему и подсвечивает (если исходное не загружено — подсказка «Показать раньше»). На удалённое ответить нельзя; если исходное удалили потом, цитата — «Сообщение удалено».
