...
| Когда можно править состав | в NEW, IN_PROGRESS, AWAITING_PAYMENT — менеджер; после — только HEAD/ADMIN; в COMPLETED/CANCELLED — никто                                                                      | —             |
| Резерв остатка             | ставится при выходе из IN_PROGRESS вперёд (в AWAITING_PAYMENT или ASSEMBLY), снимается при CANCELLED и при возврате в IN_PROGRESS; при SHIPPED списывается из stock и reserved | —             |
| Нехватка остатка           | не блокирует, а показывает предупреждение в карточке; товары «под заказ» не резервируются                                                                                      | уточнить      |
...
| Автопереход в PAID         | когда в AWAITING_PAYMENT сумма платежей ≥ итога; в журнале автор — система                                                                                                     | —             |
| Сопоставление клиента      | сначала по нормализованному телефону, потом по email (без учёта регистра); найденного клиента не перезаписываем — новые данные пишем в заказ                                   | —             |
...
| Возврат в работу           | AWAITING_PAYMENT → IN_PROGRESS разрешён (клиент меняет состав); отмена из PAID и ASSEMBLY — только HEAD/ADMIN                                                                  | —             |
...
- [ ] Вкладки-виды: «Все», «Мои», «Без менеджера», «К отгрузке» (PAID + ASSEMBLY), «Просроченные»; у вкладок — счётчики.
- [ ] Склад по умолчанию открывает «К отгрузке», остальные — «Все».
...
- [ ] Оплата: форма «способ, сумма (по умолчанию — остаток к оплате), дата, № документа»; после полной оплаты заказ в AWAITING_PAYMENT сам переходит в PAID.
- [ ] Доставка: способ, ТК, адрес, стоимость, трек; переход в SHIPPED для CARRIER требует трек-номер.
- [ ] Каждое изменение появляется в ленте: кто, когда, что было → стало.
- [ ] Склад видит карточку без кнопок правки состава и цен.
- [ ] Кнопки «Счёт PDF» и «Комплектовочный лист PDF» скачивают документ с текущими данными (реквизиты продавца — из настроек; уточнить).
...
- [ ] Сброс пароля по ссылке на email (отправка почты — SMTP из env).
...
stateDiagram-v2
    ...
    AWAITING_PAYMENT --> IN_PROGRESS: клиент меняет состав
    ...
    PAID --> CANCELLED: только HEAD/ADMIN
    ASSEMBLY --> CANCELLED: только HEAD/ADMIN
    COMPLETED --> [*]
    CANCELLED --> [*]
```
